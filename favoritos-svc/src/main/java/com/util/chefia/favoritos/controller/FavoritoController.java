...
public class FavoritoController {
    ...
    public List<FavoritoResponse> listar(@AuthenticationPrincipal Jwt jwt) {
        return service.listar(jwt.getSubject()).stream().map(mapper::aResponse).toList();
    }

    @PostMapping("/{recetaId}")
    public ResponseEntity<FavoritoResponse> agregar(@AuthenticationPrincipal Jwt jwt, @PathVariable Long recetaId) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(mapper.aResponse(service.agregar(jwt.getSubject(), recetaId, jwt.getTokenValue())));
    }

    @DeleteMapping("/{recetaId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void eliminar(@AuthenticationPrincipal Jwt jwt, @PathVariable Long recetaId) {
        service.eliminar(jwt.getSubject(), recetaId);
    }
}
