...
public class RecomendacionCacheServiceImpl implements RecomendacionCacheService {
  ...
  @Override
  @Transactional
  public void guardar(String usuarioSub, Animo animo, TipoReceta tipoReceta, String preferencia,
      List<Item> recomendaciones) {
    try {
  ...
  public List<HistorialResponse> listar(String usuarioSub) {
    return repository.findByUsuarioSubOrderByCreadoEnDesc(usuarioSub).stream()
        .map(cache -> new HistorialResponse(cache.getId(), Animo.valueOf(cache.getAnimo()),
            cache.getTipoReceta() == null ? null : TipoReceta.valueOf(cache.getTipoReceta()), cache.getPreferencia(),
            leerRecomendaciones(cache), cache.getCreadoEn()))
        .toList();
  }
