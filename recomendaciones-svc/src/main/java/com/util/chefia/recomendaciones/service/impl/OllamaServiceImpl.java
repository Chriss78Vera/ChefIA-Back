...
public class OllamaServiceImpl implements OllamaService {
 ...
 public OllamaServiceImpl(WebClient.Builder builder,ObjectMapper mapper,
  ...
  this.client=builder.baseUrl(baseUrl).build();this.mapper=mapper;this.model=model;
 }
 @CircuitBreaker(name="ollama")
 @Retry(name="ollama")
 @Override
 public List<Item> recomendar(Animo animo,TipoReceta tipoReceta,UsuariosClient.Preferencias pref,List<RecetasClient.RecetaCandidata> recetas,int cantidad){
  String prompt=""" 
   ...
   Candidatas: %s
   """.formatted(cantidad,cantidad,tipoReceta,animo,pref.tipoAlimentacion(),pref.restricciones(),pref.ingredientesNoDeseados(),recetas);
  ...
  if(response==null||response.get("response")==null)throw new IllegalStateException("Ollama no devolvio contenido");
  try{
   AiPayload parsed=mapper.readValue(aAscii(String.valueOf(response.get("response"))),AiPayload.class);
   if(parsed.recomendaciones()==null||parsed.recomendaciones().size()!=cantidad)throw new IllegalStateException("Ollama devolvio una cantidad incorrecta de recetas");
   ...
   if(nombresUnicos!=cantidad)throw new IllegalStateException("Ollama debe devolver nombres diferentes");
   boolean tiposValidos=parsed.recomendaciones().stream().allMatch(i->i.tipoReceta()==tipoReceta && esCompatible(i,tipoReceta));
   if(!tiposValidos){
   ...
   return parsed.recomendaciones().stream().map(i->new Item(i.nombre(),i.motivo(),i.descripcion(),i.porciones(),
    i.tiempoMinutos(),i.dificultad(),i.ingredientes(),i.pasos(),i.tags(),null,"OLLAMA",false,tipoReceta)).toList();
 }catch(Exception ex){throw new IllegalStateException("Respuesta JSON de Ollama invalida",ex);}
 }
 ...
 private Map<String,Object> schema(int cantidad,TipoReceta tipoReceta){
  Map<String,Object> item=Map.of(
   ...
   "required",List.of("nombre","motivo","descripcion","porciones","tiempoMinutos","dificultad","ingredientes","pasos","tags","tipoReceta"),
   "properties",Map.of(
    ...
    "ingredientes",Map.of("type","array","minItems",1,"maxItems",7,"items",Map.of("type","string")),
 ...
 private boolean esCompatible(Item item,TipoReceta tipo){
  String texto=(item.nombre()+" "+item.descripcion()+" "+String.join(" ",item.tags())).toLowerCase(Locale.ROOT);
  List<String> claves=switch(tipo){
   case DESAYUNO->List.of("desayuno","avena","huevo","tostada","pancake","cereal","yogur","arepa","waffle");
   case ENTRANTE->List.of("entrante","ensalada","bruschetta","bocado","aperitivo","tapa","rollito","ceviche","carpaccio");
   case SOPA->List.of("sopa","caldo","crema","consome");
   case PLATO_FUERTE->List.of("arroz","pollo","carne","pasta","pescado","lasania","guiso","curry","plato fuerte");
   case POSTRE->List.of("postre","flan","pastel","tarta","mousse","helado","brownie","galleta","pudin","dulce","fruta","tiramisu","chocolate","cheesecake","bizcocho","cupcake","arroz con leche","crema catalana");
  };
