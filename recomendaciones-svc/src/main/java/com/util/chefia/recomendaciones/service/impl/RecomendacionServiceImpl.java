...
public class RecomendacionServiceImpl implements RecomendacionService {
 ...
 public RecomendacionServiceImpl(UsuariosClient usuarios, CatalogoResilienteService catalogo,
  ...
  this.ollama = ollama;
  this.cache = cache;
  this.recetas=recetas;this.favoritos=favoritos;this.mapper=mapper;
 }
 ...
 public Response recomendar(Animo animo,TipoReceta tipoReceta,String usuarioSub,String accessToken){
  ...
  try {
   ...
   if(publicas.size()>=2){
    var respaldo=publicas.stream().limit(3).map(r->mapper.desdeCandidata(r,"Compatible con tu animo")).toList();
  ...
  if(!publicas.isEmpty())respuesta.add(mapper.desdeCandidata(publicas.get(0),"Receta publica compatible con tu animo"));
 ...
 private List<RecetasClient.RecetaCandidata> obtenerPublicas(Animo animo,TipoReceta tipoReceta,String tipo,String token){
  try{
