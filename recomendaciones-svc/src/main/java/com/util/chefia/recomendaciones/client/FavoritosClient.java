...
public class FavoritosClient {
    ...
    public FavoritosClient(WebClient.Builder builder, @Value("${services.favoritos.url}") String url) {
    ...
    public Set<Long> ids(String token) {
        List<Favorito> lista = client.get().uri("/api/favoritos").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .retrieve().bodyToMono(new ParameterizedTypeReference<List<Favorito>>() {
                }).block();
        if (lista == null)
            return Set.of();
        Set<Long> ids = new HashSet<>();
        lista.forEach(f -> ids.add(f.recetaId()));
        return ids;
    }
